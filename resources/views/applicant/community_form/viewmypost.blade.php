        ...
        <div class="post-header">
          <strong>
            @if(!empty($singlePost->workBackground->profileimage_path))
              <img src="{{ asset('storage/' . $singlePost->workBackground->profileimage_path) }}" alt="Profile Image" style="width:40px; height:40px; border-radius:50%; object-fit:cover; vertical-align:middle; margin-right:8px;">
            @endif
            {{ $singlePost->personalInfo->first_name ?? 'Unknown' }} {{ $singlePost->personalInfo->last_name ?? '' }}
          </strong>
          ...
          <div class="post-media">
            @if ($singlePost->image_path)
              @if ($str::endsWith($singlePost->image_path, ['.jpg', '.jpeg', '.png']))
                <img src="{{ asset('storage/' . $singlePost->image_path) }}" alt="Post Media" style="width: 100%; height: 80%;" />
              @elseif ($str::endsWith($singlePost->image_path, ['.mp4', '.mov', '.avi']))
                <video controls width="100%">
                  <source src="{{ asset('storage/' . $singlePost->image_path) }}" type="video/mp4" />
        ...
        <div class="post-actions">
          <form action="{{ route('applicant.forum.likes.store', $singlePost->id) }}" method="POST">
            ...
            <button type="submit" class="btn-like" aria-label="Like post">
              👍 <span style="{{ $hasLiked ? 'font-weight: bold;' : '' }}">Likes:</span> {{ $singlePost->likes->count() }}
            </button>
          </form>
        </div>
...
<button type="button" class="toggle-comments-btn">View Comments ({{ $singlePost->comments->count() }})</button>
    ...
    @if(session('applicant_id'))
      ...
      <form method="POST" action="{{ route('applicant.forum.comments.store') }}">
        ...
        <input
          ...
          autocomplete="off"
        />
        <button type="submit" class="comment-submit-btn" aria-label="Add Comment">Add</button>
      </form>
    @else
        ...
        <li style="margin-bottom: 1rem;">
          ...
          @if(session('applicant_id') == $comment->applicant_id)
            ...
            >
              @csrf
              @method('DELETE')
              <button type="submit" style="margin-left: 1rem; font-size: 0.9rem; color: red;">Delete</button>
            </form>
          @endif
            ...
            <li>
                ...
                @if(session('applicant_id') == $reply->applicant_id)
                    ...
                    >
                        @csrf
                        @method('DELETE')
                        <button type="submit" style="margin-left: 1rem; font-size: 0.9rem; color: red; background: none; border: none; cursor: pointer;">
                            Delete
                        </button>
                    </form>
                @endif
...
@endif
  ...
  @endif
        ...
        @if($applicantId && $applicantId == $singlePost->applicant_id)
          ...
          <form action="{{ route('applicant.forum.delete', $singlePost->id) }}" method="POST" class="mt-2 text-end">
            @csrf
            @method('DELETE')

            <!--- for edit button -->
         <div class = "viewmypost">
    <a href="{{ route('applicant.forum.editpost.display', $singlePost->id) }}" class="btn btn-warning">Edit</a>

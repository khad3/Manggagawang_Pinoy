    ...
    <div class="container container-custom">
        <div class="post-form">
            ...
            @foreach ($retrievedPosts as $post)
                <form method="POST" action="{{ route('applicant.forum.update', ['id' => $post->id]) }}"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="postTitle" value="{{ $post->title }}"
                            name="post_title" required maxlength="100" />
                    </div>
                    ...
                    @if ($post->image_path)
                        <div class="mb-3 current-media">
                            ...
                            @if (Str::endsWith($post->image_path, ['.mp4', '.mov']))
                                <video src="{{ asset('storage/' . $post->image_path) }}" controls width="100%"
                                    class="rounded"></video>
                            @else
                                <img src="{{ asset('storage/' . $post->image_path) }}" alt="Current Media"
                    ...
                    <div class="mb-3">
                        <label for="postMedia" class="form-label"><i class="bi bi-upload"></i> Upload New Photo or
                            Video</label>
                    ...
                    <div class="mb-3">
                        <label for="postContent" class="form-label"><i class="bi bi-chat-left-text"></i> Post
                            Content</label>
                        <textarea class="form-control" id="postContent" name="post_content" rows="5" required maxlength="800">{{ $post->content }}</textarea>
                    </div>

                    <button type="submit" class="btn btn-success w-100"><i class="bi bi-check-circle"></i> Update
                        Post</button>
                </form>
            @endforeach

            <div class="text-center back-button">
                <a href="{{ route('applicant.forum.display') }}" class="btn btn-primary"><i
                        class="bi bi-arrow-left"></i> Back to Forum</a>
            </div>
        </div>
    </div>

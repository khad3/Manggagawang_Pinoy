...
<body>
    ...
    <div class="top-nav">
        ...
        <div class="user-info">
            ...
            <a href="{{ route('applicant.profile.display') }}" class="name"><span
                    class="name">{{ $retrievedApplicantInfo->username }}</span></a>
        </div>
    </div>
            ...
            <div class="friends-section" id="friendsSection">
                <div class="friends-header">
                    <h3>
                        ...
                        <span id="totalNotificationBadge" class="header-notification-badge"
                    ...
                    @forelse ($friendRequests as $req)
                        <div class="card friend-request-card p-2 d-flex flex-row align-items-center mb-2 shadow-sm">
                            ...
                            <div class="flex-grow-1">
                                <strong>{{ $req->sender->personal_info->first_name }}
                                    {{ $req->sender->personal_info->last_name }}</strong><br>
                                <small
                                    class="text-muted">{{ $req->sender->is_online ? 'Online now' : 'Offline' }}</small>
                            </div>

                            <div class="ms-auto">
                                <form method="POST" action="{{ route('applicant.forum.friend.accept', $req->id) }}"
                                    class="d-inline">
                                    @csrf
                                    @method('PUT')
                                    <button class="btn btn-success btn-sm me-1">Accept</button>
                                </form>
                                <form method="POST" action="{{ route('applicant.forum.friend.cancel', $req->id) }}"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
                ...
                <div class="friends-list" id="friendsList">
                    @foreach ($retrievedFriends as $index => $friend)
                        @php
                            ...
                            $unreadCount = 0;
                            if ($friendUser) {
                                $unreadCount = $retrievedMessages
                                    ->where('sender_id', $friendUser->id)
                            ...
                            <a href="?friend_id={{ $friendUser->id }}"
                                ...
                                <div class="friend-avatar">
                                    ...
                                    @php
                                        ...
                                        $isOnline =
                                            $lastSeen && \Carbon\Carbon::parse($lastSeen)->gt(now()->subMinutes(1));
                                    @endphp
                                ...
                                <div class="friend-details">
                                    ...
                                    <small class="text-muted">
                                        @if ($unreadCount > 0)
                                            <strong>{{ $unreadCount }} new
                                                message{{ $unreadCount > 1 ? 's' : '' }}</strong>
                                        @elseif ($isOnline)
                                            Online now
                                        @else
                                            Offline
                    ...
                    <div class="chat-header">
                        ...
                        <div class="chat-user-info">
                            ...
                            <div>
                                <h6>
                                    <a
                                        href="{{ route('applicant.getprofile.display', ['id' => $selectedFriend->id]) }}">
                                        {{ $selectedFriend->personal_info->first_name }}
                                        {{ $selectedFriend->personal_info->last_name }}
                                    </a>
                                </h6>
                    ...
                    <div class="messages-container" id="messagesContainer">
                        ...
                        <div id="typingIndicator" class="typing-indicator">
                            <div class="typing-text">
                                <span id="typingUsername">{{ $selectedFriend->personal_info->first_name }}</span> is
                                typing
    ...
    <script>
        ...
        function updateNotificationSummary() {
            ...
            if (totalUnread > 0) {
                ...
                summaryText.textContent =
                    `You have ${totalUnread} unread message${totalUnread > 1 ? 's' : ''} from ${friendsWithMessages} friend${friendsWithMessages > 1 ? 's' : ''}`;
        ...
        function pollFriendTyping() {
            ...
            fetch("{{ route('applicant.check.typing', ['receiver_id' => '__RECEIVER_ID__']) }}".replace('__RECEIVER_ID__',
                    selectedFriendId), {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json'

...
<div class="container py-5">
    ...
    <div class="d-flex justify-content-between align-items-center">
      <div>
        ...
        <div class="group-meta">
          <span>👤 <strong>{{ $group->personalInfo->first_name ?? 'Unknown' }} {{ $group->personalInfo->last_name ?? '' }}</strong></span>
          <span>🔒 <span class="badge {{ $group->privacy === 'public' ? 'bg-light text-dark' : 'bg-dark' }}">{{ ucfirst($group->privacy) }}</span></span>
          <span>👥 <strong>{{ $group->members_count }} Members</strong></span>
        </div>
      </div>
      <div>
        <a href="{{ route('applicant.forum.groupcommunity.display') }}" class="btn btn-outline-light me-2">← Back</a>
        <button class="btn btn-light text-primary" data-bs-toggle="modal" data-bs-target="#newPostModal">+ New Post</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="newPostModal" tabindex="-1" aria-labelledby="newPostModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form method="POST" action="{{ route('applicant.forum.addpostgroup.store' , ['groupId' => $group->id]) }}" enctype="multipart/form-data">
          @csrf
  ...
  <ul class="nav nav-tabs group-tabs" id="groupTab" role="tablist">
    ...
    @if($group->applicant_id == $applicantId)
      <li class="nav-item" role="presentation">
        <button class="nav-link" id="pending-tab" data-bs-toggle="tab" data-bs-target="#pending" type="button" role="tab">Pending Members</button>
      </li>
    @endif
    @endif
  </ul>
      ...
      @forelse($retrievePosts as $post)
        <div class="post-card">
          ...
          <p class="text-muted mb-0">
            Posted by {{ $post->personalInfo->first_name ?? 'Unknown' }} {{ $post->personalInfo->last_name ?? '' }} • 
...
<form method="POST" action="{{ route('applicant.forum.groupaddlike.store', ['groupId' => $group->id, 'postId' => $post->id]) }}">
    @csrf
    <button type="submit" class="btn btn-sm btn-outline-primary me-2">
        👍 Like ({{ $post->likes->count() ?? 0 }})
              ...
              <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#comments-{{ $loop->iteration }}">💬 Comments ({{ $post->comments->count() }}) </button>
            </div>
            <span class="text-muted small">Last updated: {{ \Carbon\Carbon::parse($post->updated_at)->diffForHumans() }}</span>
          </div>
  ...
  <div class="d-flex mb-3">
   <div class="me-3">
      @if($comment->applicant && $comment->applicant->work_background && $comment->applicant->work_background->profileimage_path)
    ...
    <div class="flex-grow-1 bg-light p-3 rounded shadow-sm">
      <div class="d-flex justify-content-between align-items-center mb-1">
        <strong>{{ $comment->personal_info->first_name ?? 'Unknown' }} {{ $comment->personal_info->last_name ?? '' }}</strong>
     ...
     <form action="{{ route('applicant.forum.groupcomments.delete', ['groupId' => $group->id, 'commentId' => $comment->id]) }}" method="POST" class="mt-2">
    @csrf
    @method('DELETE')
...
@endforeach
              ...
              <form action="{{ route('applicant.forum.groupcomments.store', ['groupId' => $group->id]) }}" method="POST">
                @csrf
                <input type="hidden" name="per_group_community_post_id" value="{{ $post->id }}">
    ...
    <div class="tab-pane fade" id="members" role="tabpanel">
      <div class="member-item">
        <strong>{{ $group->personalInfo->first_name ?? 'Unknown' }} {{ $group->personalInfo->last_name ?? '' }}</strong>
        (Group Creator)

...
<body>
  ...
  <div class="container py-5">
    ...
    @forelse($listOfGroups as $group)
      <div class="group-card mb-4">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap group-meta">
          <a href="{{ route('applicant.forum.creatorviewpage.display', $group->id) }}" class="btn btn-outline-primary btn-custom">View Group</a>

          <form action="{{ route('applicant.forum.deletegroupcreator.delete', $group->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this group?');">
            @csrf
            @method('DELETE')
    ...
    <div class="back-btn text-center">
      <a href="{{ route('applicant.forum.groupcommunity.display') }}" class="btn btn-primary btn-lg">← Back to Forum Page</a>
    </div>
  </div>

              ...
              @if($retrievedProfile->id != session('applicant_id'))
                    @if ($retrievedProfile->id !== session('applicant_id'))
                        ...
                        @if (!$retrievedProfile->alreadySent)
                            <form method="POST" action="{{ route('applicant.forum.addfriend.store', ['id' => $retrievedProfile->id]) }}">
                                @csrf
                        ...
                        @elseif ($retrievedProfile->alreadySent && $retrievedProfile->friendRequestStatus === 'pending' && $retrievedProfile->isReceiver)
                            <form method="POST" action="{{ route('applicant.forum.friend.accept', ['id' => $retrievedProfile->friendRequestId]) }}">
                                @csrf
                                @method('PUT')
                        ...
                        @elseif ($retrievedProfile->alreadySent && $retrievedProfile->friendRequestStatus === 'pending')
                            <form method="POST" action="{{ route('applicant.forum.friend.cancel', ['id' => $retrievedProfile->friendRequestId]) }}">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-outline-danger mt-2">Cancel Friend Request</button>
                            </form>

                      {{-- If already friends --}}
@elseif ($retrievedProfile->friendRequestStatus === 'accepted')
    <form action="{{ route('applicant.unfriend.store', $retrievedProfile->id) }}" method="POST" class="d-inline">
        @csrf
        @method('DELETE')
        <button type="submit" 
                class="btn btn-success position-relative friend-btn">
            You are friends
        </button>
    </form>
@endif
...
<div class="content-card animate-slide-in">
  <div class="card-header-tesda d-flex justify-content-between align-items-center">
    ...
    <div class="dropdown">
      <button class="btn btn-sm btn-tesda btn-primary-tesda dropdown-toggle" data-bs-toggle="dropdown">
        <i class="bi bi-plus-circle me-1"></i>Add
      </button>
      <ul class="dropdown-menu">
        <li>
          <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#uploadImageModal">
            <i class="bi bi-image me-2"></i>Upload Image</a>
        </li>
        <li>
          <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#addYoutubeModal">
  ...
  <div class="card-body-tesda">
    ...
    <div class="portfolio-item mb-3" data-bs-toggle="modal" data-bs-target="#viewSampleWorkModal{{ $portfolio->id }}">
      <img src="{{ asset('storage/' . $portfolio->sample_work_image) }}" alt="Sample Work" class="img-fluid" />
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body p-0">
            <img src="{{ asset('storage/' . $portfolio->sample_work_image) }}" class="w-100 rounded"
              alt="Sample Work Enlarged" />
          </div>
          <div class="modal-footer justify-content-between">
            <span class="text-muted">
              {{ $portfolio->sample_work_title }} – {{ $portfolio->sample_work_description }}
            </span>
    ...
    <div class="portfolio-item mb-3">
      <div class="ratio ratio-16x9">
        <iframe src="{{ $embedUrl }}" title="{{ $youtube->sample_work_title }}" allowfullscreen
                  ...
                  <ul class="dropdown-menu">
                    <li>
                        @if (session('applicant_id') == $retrievedProfile->id)
                            ...
                            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#viewCertificationModal">
            ...
            <div class="modal-body">
              <div class="mb-3">
                ...
                <small class="text-muted"
                  >Accepted: PDF, JPG, PNG (Max 5MB)</small
                >
              </div>
              <div class="mb-3">
